...
class SWTLoss(nn.Module):
    ...
    #================================================================
        ...
        if c == 3:
            # RGB转Y通道（保持原逻辑）
            sr_img_y = 16.0 + (pred[:, 0:1] * 65.481 + pred[:, 1:2] * 128.553 + pred[:, 2:3] * 24.966)
            hr_img_y = 16.0 + (target[:, 0:1] * 65.481 + target[:, 1:2] * 128.553 + target[:, 2:3] * 24.966)
        elif c == 1:
            # 单通道直接使用
            sr_img_y = pred
            hr_img_y = target
        else:
            raise ValueError(f"Unsupported channel number {c}, expected 1 or 3")
